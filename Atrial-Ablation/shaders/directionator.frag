...
#define texelInDomain(I)  \
    ( texelFetch(compressedTexelIndex,(I), 0).a==uint(1) )

#define inDomain( v )   (texelInDomain( getIJ(v, size) )) 

#define getTargetIndex(v) \
    ivec2(texelFetch(compressedTexelIndex,getIJ(v,size),0).xy)

#define ablated(v)    \
    (texelFetch( ablationMap,getTargetIndex(v),0).r>0.5)

#define isNotGood(v)   \
    (!( inDomain(v) && isInBounds( v )) || ablated(v))
...
void main(){
    ...
    // check if the point is ablated itself ..............................
    if ( ablated(cidx) ) return ;
